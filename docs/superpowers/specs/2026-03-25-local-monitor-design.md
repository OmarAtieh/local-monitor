...
| Category | Metrics |
|----------|---------|
| CPU | Per-core utilization %, overall %, frequency (GHz), temperature (°C) |
| RAM | Used/total (GB), utilization %, swap used/total (GB) |
| GPU | Utilization %, temperature (°C), fan speed (RPM), clock speed (MHz) |
| VRAM | Used/total (GB), utilization % |
| Disk | Per-drive capacity used/total, read/write IO rates (MB/s) |
| Network | Upload/download rates (KB/s or MB/s), total bytes transferred |
...
2. GPU & VRAM — GPU utilization + temperature overlay / VRAM utilization
...
Only granularities with available data are selectable (e.g., 7d is only available after 7 days of uptime).
...
- SQLite database auto-created alongside the executable (`localmonitor.db`)
- Raw samples stored at 1-second resolution for the most recent minute
- Downsampled averages for longer windows:
  - 5s averages retained for 30 minutes
  - 30s averages retained for 4 hours
  - 5m averages retained for 24 hours
  - 15m averages retained for 7 days
